...
public class ImageCapturer extends Thread {
    ...
    public enum State {WORKING, SIZE, IMAGE};
     ...
     * @throws IOException
     */
    public ImageCapturer() throws IOException {
		if (DEBUG) System.out.println("New ImageCapturer ...");
    ...
    public void run () {
        while (running) {
            ...
            if (DEBUG) System.out.println("ImageCapturer: State: " + captureState);
    ...
    public void awaitDesiredState(State desiredState) {   	   	
		if (DEBUG) System.out.println("ImageCapturer: about to wait state change to " + desiredState);
		if (desiredState == captureState) { // already in state so just return
			...
			while (true) {
				...
				if (myState == desiredState) {
					if (DEBUG) System.out.println("ImageCapturer: got state change to " + myState);
					break;
				} 					
			}
		}
	...
	public static void main(String[] args) throws IOException {
		...
		// get a new capturer
		ImageCapturer ic = new ImageCapturer();
		ic.start();
